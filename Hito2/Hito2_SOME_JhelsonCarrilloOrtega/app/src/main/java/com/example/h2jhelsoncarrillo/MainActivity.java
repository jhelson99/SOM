        ...
        switch (v.getId())
        {
            ...
            case R.id.btnBuscar:
                ...
                if(caso.equals("CONFIRMADOS"))
                {
                    ...
                    {
                        Toast.makeText(this.getApplication(), "COCHABAMBA: " + numCCocha, Toast.LENGTH_LONG).show();
                    }
                    else if(numCSC > numCC && numCSC > numCO)
                    {
                        Toast.makeText(this.getApplication(), "SANTA CRUZ: " + numCSanta, Toast.LENGTH_LONG).show();
                    }
                    else if(numCO > numCSC && numCO > numCC)
                    {
                        Toast.makeText(this.getApplication(), "ORURO: " + numCOruro, Toast.LENGTH_LONG).show();
                    }
                }
                else if(caso.equals("SOSPECHOCOS"))
                {
                    if(numSC > numSSC && numSC > numSO)
                    {
                        Toast.makeText(this.getApplication(), "COCHABAMBA: " + numSCocha, Toast.LENGTH_LONG).show();
                    }
                    else if(numSSC > numSC && numSSC > numSO)
                    {
                        Toast.makeText(this.getApplication(), "SANTA CRUZ: " + numSSanta, Toast.LENGTH_LONG).show();
                    }
                    else if(numSO > numSSC && numSO > numSC)
                    {
                        Toast.makeText(this.getApplication(), "ORURO: " + numSOruro, Toast.LENGTH_LONG).show();
                    }
                }
